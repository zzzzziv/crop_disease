...
source_dir = 'data/AgriculturalDisease_validationset/images/'
img = os.listdir(source_dir)  # 得到文件夹下所有文件名称

with open("AgriculturalDisease_validation_annotations.json", 'r') as load_f:
    load_dict = json.load(load_f)
    # print(load_dict)

for pop_dict in load_dict:
    path = "./val_data/" + str(pop_dict['disease_class'])
    mkdir("./val_data/" + str(pop_dict['disease_class']))  # 根据类别创建文件夹
    # print(path)
    for fileNum in img:
